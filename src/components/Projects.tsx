  ...
  return (
    <article className="flex flex-col-reverse flex-wrap justify-between lg:justify-normal min-w-[85%] max-w-[40%] lg:max-w-full lg:flex-row items-center relative m-[5%] lg:m-0 lg:p-[7%] z-10 lg:py-[6%] bg-[#111111c0] lg:bg-transparent rounded-md shadow-[8px_5px_15px_-6px_#2d3748] lg:shadow-none">
      <div className="w-full lg:w-[35vw] gap-6 flex flex-col p-4 lg:p-0">
        <p className="text-lg pt-4 lg:pt-0 text-gray-300">{`PROJECT #${
          project.id.split("-")[1]
        }`}</p>
        <h3 className="text-xl font-semibold">{project.title}</h3>
        <div className="lg:bg-[#111111c0] lg:rounded-md lg:p-4 lg:shadow-[8px_5px_15px_-6px_#2d3748]">
          <p className="lg:text-lg text-gray-200">{project.description}</p>
        </div>
        <div
          className="flex gap-1 lg:gap-6 flex-wrap md:w-[120%]"
          role="list"
        ...
        >
          {project.techStack.map((tech: string, index: number) => (
            <span
              ...
              className="bg-[#111111d7] p-2 px-5 rounded-full shadow-[8px_3px_15px_-6px_#2d3748] text-sm"
              role="listitem"
            >
              {tech}
            </span>
